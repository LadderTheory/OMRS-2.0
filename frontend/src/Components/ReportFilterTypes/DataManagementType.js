    ...
    return(
        <div className='form-group'>
            ...
            <select className= 'form-control mb-1' id={props._id} name={props.selectedFilter} onChange={inputChange} >
                <option>Select a(n) {props.selectedFilter}:</option>{data.filter(filteredData => filteredData.active === true).map((filter) => (<option key={filter._id} value={filter._id}>{filter.name}</option>))}
            </select>
        </div>
    )
}
